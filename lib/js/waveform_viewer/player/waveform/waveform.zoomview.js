...
define([
  ...
  function WaveformZoomView(waveformData, $container, options) {
    ...
    bootstrap.pubsub.on("waveform_zoom_level_changed", function (zoom_level) {
      ...
      if (zoom_level != that.current_zoom_level) {
        ...
        // index in zoom level array of sample rate
        that.oldZoomLevel = that.current_zoom_level;
        that.current_zoom_level = zoom_level;

        // actual sample rate
        that.old_sample_rate = options.zoomLevels[that.oldZoomLevel];
  ...
  WaveformZoomView.prototype.startZoomAnimation = function (pixelIndex) {
    ...
    var frameData = [];
    var diff = that.current_zoom_level - that.oldZoomLevel;
    console.log("current_sample_rate", that.current_sample_rate, "that.old_sample_rate", that.old_sample_rate);
    for (var i = 0; i < 60; i++) {
      var frame_sample_rate = that.old_sample_rate + Math.round((that.current_sample_rate - that.old_sample_rate)*((i+1)/60)); // Work out interpolated resample scale using that.current_zoom_level and that.oldZoomLevel and wether you are zooming in or out
      frameData.push(
    ...
    that.zoomAnimation = new Kinetic.Animation(function (frame) {
      ...
      if (frameData.length > 0) {
        ...
        var timecode_pixel_index = myData.at_time(that.currentTime);

        console.log(myData.adapter.data.sample_rate);

        // Send correct data from frameData to drawFunc for waveform and draw it
        that.zoomWaveformShape.setDrawFunc(function(canvas) {
          myData.offset(timecode_pixel_index, timecode_pixel_index + that.width);

          mixins.waveformDrawFunction.call(this, myData, canvas, mixins.interpolateHeight(that.height));
        });
